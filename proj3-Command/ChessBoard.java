...
import static assignment03.BoardColors.CLICKED;
...
// This page provided code to show chess pieces using unicode:
// https://stackoverflow.com/questions/18686199/fill-unicode-characters-in-labels
// A notation for chess games:
// https://en.wikipedia.org/wiki/Algebraic_notation_(chess)
// The pure numeric notation used in this assignment:
...
class ChessBoard {
		...
		l.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				if(fromClick) {
					...
					if(board.getICCF(toICCF).getColor() != BoardColors.NONE)
	...
	public static JPanel drawGui(JFrame frame, Board board) {
		...
		for (int r = 8; r >= 1; r--) {
			...
			for (int f = 1; f <= 8; f++) {
				Piece piece = board.getICCF(10*f+r);
				if (piece.getColor() == BoardColors.NONE) {
					addColoredUnicodeCharToContainer(
							" ", gui, new Color(203,203,197),
							Color.DARK_GRAY,
							frame, board, f, r);
				} else {
					if(piece.getColor() == BoardColors.WHITE)
						addColoredUnicodeCharToContainer(
								piece.getCode(), gui, new Color(203,203,197),
								Color.DARK_GRAY,
								frame, board, f, r);
					else 
						addColoredUnicodeCharToContainer(
								piece.getCode(), gui, new Color(192,142,60),
								Color.DARK_GRAY,
								frame, board, f, r);
				}
			}
